...
typedef enum {
  ...
  SUB_8, // pop two values off the stack and subtract them, e.g. subtracted = top, subtractee = top - 1 and push the result
  MUL_8, // pop two values of the stack, multiply them and push the result 
  DIV_8,  // divide the values on top of the stack, e.g. divider = top, divident = top - 1
  PUSH_8, // push a value
  POP_8, // pop a value
  ...
  PRT_8,  // push the value on top at an address specified by an offset to the top, e.g. prt8 1 addresses top - 1
  ...
  JZ_8,       // jump if comparison result is zero, e.g. a != b will give a zero result
  JNZ_8,      // jump if comparison result is non-zero, e.g. a == b will give a non-zero result
  CALL_8,     // call a function at label
  RET_8,	  // return the value on top of the stack and push it on top of the caller stack frame
  NOP,        // no-op
} OPCODE;
